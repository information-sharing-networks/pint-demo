...
func (current *CurrentTransportDocumentState) CanAccept(next requestedAction, thisPlatformCode string) (accept bool, reason string, error error) {
	...
	if !current.Accepted {
		return false, fmt.Sprintf("previous action (%s) is in progress and not yet accepted for this eBL", current.ActionCode), nil
	}

	// handle surrender actions (these block further actions)
	switch current.ActionCode {

	// a previously surrendered eBL block all actions unitl the carrier responds
	...
	// accepted surrender blocks all further actions
	case ebl.ActionCodeSACC:
		return false, "eBL already surrendered, no further action permitted", nil

	// transfers block further actions from the same platform
	case ebl.ActionCodeTransfer:
		if current.ReceivedByPlatformCode == next.PlatformCode {
			return false, fmt.Sprintf("transfer was already acccepted from %s", current.ReceivedByPlatformCode), nil
		}
	}

	// check if the next action is compatible with the current state of the eBL
	switch next.ActionCode {
	...
	case ebl.ActionCodeTransfer:
		...
		case next.PlatformCode != thisPlatformCode:
			return false, "internal error", fmt.Errorf("%s cannot proccess a transfer addressed to: %s", thisPlatformCode, next.PlatformCode)

		// the current platform must be the recipient of the last accepted transfer
		case current.ReceivedByPlatformCode != thisPlatformCode:
			return false, "internal error", fmt.Errorf("internal error: the current platform %s is not the recipient of the last accepted transfer (%s)", thisPlatformCode, current.ReceivedByPlatformCode)

		// transfers must be previously endorsed to the recipient platform
		case current.ActionCode != ebl.ActionCodeEndorse && current.ActionCode != ebl.ActionCodeEndorseToOrder && current.ActionCode != ebl.ActionCodeBlankEndorse:
			return false, "internal error", fmt.Errorf("transfer can only be accepted after an endorsement, (have %s)", current.ActionCode)
		}
	...
	case ebl.ActionCodeEndorse, ebl.ActionCodeEndorseToOrder, ebl.ActionCodeBlankEndorse:
		return true, fmt.Sprintf("endorsement accepted for this eBL (current state: %s)", current.ActionCode), nil
	}
	return false, "internal error", fmt.Errorf("bug: unhandled eBL transition, %s", current.ActionCode)
}
