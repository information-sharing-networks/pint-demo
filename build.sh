...
#
# Note if you want versions on your cloud deployed app, you will need to update your CD scripts to build the binaries for your platform with the version number, build date and git commit hash
...
LDFLAGS="-X github.com/information-sharing-networks/pint-demo/app/internal/version.version=$VERSION \
    -X github.com/information-sharing-networks/pint-demo/app/internal/version.buildDate=$BUILD_DATE \
    -X github.com/information-sharing-networks/pint-demo/app/internal/version.gitCommit=$GIT_COMMIT"
...
if [ ${LINUX} ]; then
    echo "Building $VERSION (linux binary)"
    GOOS=linux 
    GOARCH=amd64
 else
    echo "Building $VERSION (Mac darwin binary)"
    GOOS=""
    GOARCH=""
fi
...
echo "Build complete: see app/bin for binaries"
