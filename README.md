...
An implementation of the DCSA PINT (Platform Interoperability) API v3.0.0 for electronic Bill of Lading (eBL) transfers.
...
If you plan to make changes to the code you will also need the latest version of [Go](https://go.dev/doc/install).

### Quick Start

#### 1. create a .env file in the root of the project. You can use the example file as a starting point:
```bash
cp .env.example .env
```
...
# The keymanager will load any public key in the directory that has a matching kid in the registry (other keys will be ignored).
...
# DCSA Code of the platform this instance represents 
PLATFORM_CODE="EBL1"
...
```bash
make docker-reset # warning - this command recreates the pint dev database
```
...
Keys are looked up by the KID retrieved from JWS headers. Where the keygen CLI (see below) is used to generate the keys, the KID is the first 16 characters of the SHA-256 thumbprint of the public key in JWK format (c.f RFC7638).
...
- **Authorization**: The registry contains the list of platforms that are allowed to participate in the PINT network. This is used to prevent platforms from joining the network without first signing up to the DCSA agreement.
- **Security**: the registry contains the JWKS endpoint for each platform or - where no JWKS endpoint is specified - the KID of the manually configured key for the platform. This information is used to ensure that the public keys needed to verify JWS signatures are only retrieved from trusted locations.

This list location is configured via the `DCSA_REGISTRY_PATH` environment variable.

For the purpose of the demo implementation the registry is based on a local file (`app/test/testdata/platform-registry/eblsolutionproviders.csv`)
...
Three tiers of trust are supported:
...
**Note** the pint-server only needs the *private key JWK* - it is used to sign messages and to generate a public key JWK set to be shared with other platforms via the `/.well-known/jwks.json` endpoint. You can use an externally generated key if you prefer - it must be in JWK format and either Ed25519 or RSA.
...
If you want to include x5c, you will need to create a certificate for your PINT platform and have it signed by a Certificate Authority (CA).
...
This app does not support cert generation - use external tools for this.
...
There is also a basic implemenation of DCSA *eBL Issuance Requests*  (EBL_ISS_v3.0.2) to help create end-2-end tests.

These functions take care of all the required cryptographic steps needed to create and validate PINT and issuance requests. See the `crypto` package for the low level function that they use.
